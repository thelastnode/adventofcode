...
object Day13_1 {
    ...
    private fun step(carts: List<Cart>, grid: SortedMap<Position, Cell>): List<Cart> {
        val cartPositions = carts.map { it.pos }.toMutableSet()
        return carts.sortedBy { it.pos }.map { cart ->
            ...
            if (hasCrashed) {
                println("Cart crashed: $nextPos")
            }

            cartPositions.remove(pos)
            cartPositions.add(nextPos)

            Cart(nextPos, nextNext, if (hasCrashed) 'X' else nextRaw)
        }
    }
...
fun main(args: Array<String>) {
    ...
    val inputs = Day13_1.parse(lines)
    println(Day13_1.process(inputs))
}